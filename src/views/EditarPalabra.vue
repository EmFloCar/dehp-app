...
export default {
    ...
    methods:{
        editar(){
            axios.put("http://localhost:3000/palabra/" + this.palabraId, this.palabra).then(data=>{
                console.log(data)
            })
        },

        salir(){
           this.$router.push("/tabla-palabras") 
        },
    ...
    mounted(){
        ...
        axios.get("http://localhost:3000/palabra/" + this.palabraId).then(datos => {
